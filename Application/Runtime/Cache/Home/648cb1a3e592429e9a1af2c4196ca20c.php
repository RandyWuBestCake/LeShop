...
<div class="main">
  <div class="center">
      <div class="pro_top line_b pb5">
        <div class="left">
            <a href="/"><span class="dark font14"><strong>首页</strong> </span></a>
            <span class="font14"> > </span>
            <span class="font14 gray">全部商品</span>
        </div>
      </div>
      <div class="product_alllist">
         <?php if(is_array($res)): foreach($res as $key=>$vo): ?><div class="row">

              <div class="row_col tall">
                 <div class="v">
                    <div class="product_pic"><img  alt="<?php echo ($vo["name"]); ?>"  src="/object<?php echo ($vo["img"]); ?>" style=""/></div>
                    <h2 class="product_tit t_c"><?php echo ($vo["name"]); ?></h2>
                    <div class="product_sub pt10">
                       <ul>
                          <?php if(is_array($goods)): foreach($goods as $key=>$vo1): if(is_array($vo1)): foreach($vo1 as $key=>$vo2): if($vo['id']== $vo2['pid']): ?><li>· <a href="<?php echo U('Goods/headline',array('id'=>$vo2['id']));?>" target="_blank" webtrekkparam="{ct:'lj_sy_qbsp_max70'}"><?php echo ($vo2["name"]); ?></a></li>
                          <?php else: endif; endforeach; endif; endforeach; endif; ?>
                       </ul>
                    </div>
                 </div>
              </div><?php endforeach; endif; ?>
              <div class="clear"></div>
               <?php if(is_array($fit)): foreach($fit as $key=>$vo): ?><div class="row_col low">
                 <div class="v">
                    <div class="product_pic"><img  alt="<?php echo ($vo["name"]); ?>"  src="/object<?php echo ($vo["img"]); ?>" style=""/></div>
                    <h2 class="product_tit t_c"><?php echo ($vo["name"]); ?></h2>
                    <div class="product_sub pt10">
                       <ul>
                          <?php if(is_array($fits)): foreach($fits as $key=>$vo1): if(is_array($vo1)): foreach($vo1 as $key=>$vo2): if($vo['id']== $vo2['pid']): ?><li>· <a href="<?php echo U('Fit/FitList');?>" target="_blank" webtrekkparam="{ct:'lj_sy_qbsp_newC1S'}"><?php echo ($vo2["name"]); ?></a></li>
                            <?php else: endif; endforeach; endif; endforeach; endif; ?>
                       </ul>
                    </div>
                 </div>
