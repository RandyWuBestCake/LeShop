...
<head>
  ...
  <!-- 引用静态页面标签 -->
    ...
    <link  type='text/css' href='/object/Public/CSS/page.css' rel='stylesheet' />
    <script type='text/javascript' src='/object/Public/Scripts/zxlib.js'></script>
    <script type="text/javascript" src="/object/Public/Scripts/unitpngfix.js"></script>
      <script type='text/javascript' src='/object/Public/Scripts/newag.js'></script>
    <script type='text/javascript' src='/object/Public/Scripts/jquery.pagination.js'></script>
    ...
    <title>超级电视,乐视盒子配件频道-乐视商城-乐视商城</title>
  <meta name="keywords" content="配件,电视配件,乐视配件,盒子配件">
  <meta name="description" content="" >
    <link rel="canonical" href="http://shop.letv.com/product/parts.html"/>
...
<div class="main">
    <div class="center">
      <a href="<?php echo U('Fit/detail',array('id'=>13));?>"  target="_blank" webtrekkparam="{ct:'lj_sy_pjzq_tl'}">
        <img  alt="" src="/object/Public/Picture/61fd5137d41e4cbfb7583b09c33768a6.gif"/>
      </a>
    </div>
    <div class="product_tvlist">
        <div class="bread_nav center line_b ">
            <div class="font14">
                <a href="/" webtrekkparam="{ct:'lj_sy_pjzq_sy'}"><span class="dark"><strong>首页</strong></span></a> > <span class="gray_2">配件专区</span>
            </div>

        </div>
<!-- ********************************一级菜单栏筛选********************************************* -->
        <div class="filter center" >
          <?php if(is_array($cates1)): foreach($cates1 as $key=>$vo1): ?><div class="item">
          <label><?php echo ($vo1['name']); ?>：</label>
            <ul>
                <li class="pame7 current "><a class="allfit" style="cursor:pointer" path="<?php echo ($vo1["path"]); ?>" id="<?php echo ($vo1["id"]); ?>">全部</a><em>丨</em></li>
                <?php if(is_array($cates2)): foreach($cates2 as $key=>$vo2): if(substr($vo2['path'],-2)==$vo1['id']): ?><li class="subpame39"><a class="fit" style="cursor:pointer" id="<?php echo ($vo2["id"]); ?>"><?php echo ($vo2["name"]); ?></a><em>丨</em></li>
  ...
  <!-- ***********************************二级菜单栏筛选********************************************** -->
        <div class="sort">
            <div class="center">
                <div class="left fenlei">
                    <a class="default"><span id="sortColor0" class="sortFontColor red" name="0" style="cursor:pointer">默认</span></a>
                    <a class="new"><span id="sortColor1" class="sortFontColor" name="1">最新</span></a>

                    <a class="s_price"><span id="sortColor2" class=" sortFontColor" price="1">价格</span></a>

                </div>
                <div class="right pagenation">

                </div>
                <!-- 分页展示 -->
                <div class="pagination" id="Pagination"><?php echo ($page); ?></div>
            </div>  
        </div>
        <!--配件list begin-->
        <div class="filterresult center" id="parts_list">
          <ul id = "rep">
          <!-- ****************商品遍历************************************ -->
            <?php if(is_array($infos)): foreach($infos as $key=>$vo): ?><li>
              ...
              <div class="pro_btns" >
              <a  num="1" id="<?php echo ($vo["id"]); ?>" amount="<?php echo ($vo["amount"]); ?>" count="<?php echo ($count); ?>" type="slideTip" class="addCart gray_bt_s block_center"><span class="">加入购物车</span></a>
        ...
        <script type="text/javascript">
          ...
          $('.addCart').click(function(){
            ...
            var num = $('#productNumInCart').html();
            $('#productNumInCart').show(100);
            $.get("<?php echo U('Cart/addCart');?>",{goods_id:goods_id,cart_amount:1},function(msg){
              if(msg =='1'){
                var new_productNum = 1;
              }else{
                var new_productNum = num*1+1;
              }
          ...
          //获取附带参数值
          var path = $('.allfit').attr('path');
          var id = $('.allfit').attr('id');
          $('.allfit').click(function(){
            ...
            // ajax传值
            $.get("<?php echo U('Fit/FitListblock');?>",{id:id,path:path},function(msg){
                $('#rep').html(msg);   
            })
            //添加价格排序条件
            $('#sortColor2').toggle(
              function(){
                $('#sortColor0').removeClass();
              $('#sortColor2').parent().addClass('s_price_hover s_price_t_hover');
              $('#sortColor1').removeClass();
              $.get("<?php echo U('Fit/FitListblock');?>",{price:0},function(msg){
                  $('#rep').html(msg);
              })
           },
            function(){
              $('#sortColor2').parent().removeClass('s_price_t_hover');
              $.get("<?php echo U('Fit/FitListblock');?>",{price:1},function(msg){
                $('#rep').html(msg);
            })
            }
        );
          ...
          $('.fit').click(function(){
            var id = $(this).attr('id');
            //改变样式
            // $('.subpame39').removeClass();
            $('.current').removeClass();
            $(this).parent().addClass('current');
            $.get("<?php echo U('Fit/block');?>",{id:id},function(msg){
                $('#rep').html(msg);
            })
          })

          $('#sortColor1').click(function(){
            // var id = $('.allfit').attr('id');
            // var path = $('.allfit').attr('path');
            $('#sortColor2').parent().removeClass('s_price_hover s_price_t_hover');
            $('#sortColor0').removeClass();
            $(this).addClass('red');
            var v = $(this).attr('name');
            $.get("<?php echo U('Fit/newblock');?>",{id:v},function(msg){
                $('#rep').html(msg);
            })
          })
          $('#sortColor0').click(function(){
            $('#sortColor1').removeClass();
            $('#sortColor2').parent().removeClass('s_price_hover s_price_t_hover');
            $(this).addClass('red');
            $.get("<?php echo U('Fit/newdefault');?>",{id:id,path:path},function(msg){
                $('#rep').html(msg);
          })
        })
        $('#sortColor2').toggle(
            function(){
              $('#sortColor0').removeClass();
            $(this).parent().addClass('s_price_hover s_price_t_hover');
            $('#sortColor1').removeClass();
            $.get("<?php echo U('Fit/by');?>",{price:0},function(msg){
                $('#rep').html(msg);
            })

           },
            function(){
              $(this).parent().removeClass('s_price_t_hover');
              $.get("<?php echo U('Fit/by');?>",{price:1},function(msg){
                $('#rep').html(msg);
            })
            }
        );
...
<!-- 统计代码end--><script type="text/javascript">
  ...
  "common": {
    ...
    "bdText" : "游戏手柄打起电动有够爽，超级遥控器换台唰唰唰！没事用超级歌王吼一嗓，还有万能体感摄像头，方便的键鼠套装…都是@超级电视 的好盆友~越来越多的高能配件，无限惊喜走起来>>>", 
    "bdDesc" : "游戏手柄打起电动有够爽，超级遥控器换台唰唰唰！没事用超级歌王吼一嗓，还有万能体感摄像头，方便的键鼠套装…都是@超级电视 的好盆友~越来越多的高能配件，无限惊喜走起来>>>", 
    "bdUrl" : "http://shop.letv.com/product/parts.html?cps_id=u_p_pj",  
    "bdPic" : "http://img0.hdletv.com/file/20141103/default/51410299199395805",
    "bdMini": "2",
    "bdMiniList": false,
    "bdStyle": "0",
    "bdSize": "32"  
