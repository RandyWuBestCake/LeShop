...
<!-- 引用静态页面标签 -->
				...
				<![endif]-->
  		  		...
  		  		<script type='text/javascript' src='/object/Public/cart/Scripts/cart.js'></script>
...
<script type="text/javascript">
    ...
    <div class="center">
        <div class="cart_title mt10">
            <div class="left">
              <span class="font12 gray">
                  <span class="font20 dark">我的购物车</span>     
			
              </span>
             
          	</div>
          
          <!--<div class="right simsun">有问题？请您<a href="/onlineservice.html" target="_blank"><span class="red">联系客服</span></a></div>-->
            <div class="clear"></div>
        </div>
        <div id="tip" class="cart_tishi clear mt10 pb20" style="display:none">
            <span class="tishi_bg block left white t_c">声明</span>
            <span class="block left red">由于您的选购的商品由不同的区域仓库配送,所以您需要分别结算订单.由此给您带来的不便深表歉意.</span>
        </div>

        <?php  $province_arr = array('北京','重庆','福建','甘肃','广东','广西','贵州','海南','河北','安徽','黑龙江','河南','香港','湖北','湖南','江苏','江西','吉林','辽宁','澳门','内蒙古','宁夏','青海','山东','上海','山西','陕西','四川','台湾','天津','新疆','西藏','云南','浙江','海外'); ?>



      <?php  session_start(); if(!session('cart')){ ?>
        
      <div class="cart_noneWrap">
      <img class="cart_img mr50" src="/object/Public/cart/Images/cart9527.jpeg">
  
     <!--登录状态无商品-->
      <div id="loginCartNone" class="cart_login mt10 hidden" style="display: block;">
        <span class="cart_txt">你的购物车还是空的！ <br>赶紧行动吧！</span>
        <a href="#"><span class="shop_now">马上去购物</span></a>
      </div>
      </div>
      ...
      <div id="head" style="">
        ...
        <table border="0" cellspacing="0" cellpadding="0" class="cart_list">
     
          <th class="no_bl" width="100"><input type="checkbox" name="selectGood" class="mr10" id="allcheck" /> 全选</th>
          <th class="no_bl"></th>
          <th class="cart-pGoods no_bl">商品</th>
          <th class="cart-pPrice" width="152">单价</th>
          <th class="cart-pQuantity" width="152">数量</th>
          <th class="cart-pDiscount" width="152">小计</th>

          <th class="cart-pAction" width="152"><select class=" cart_select">

            <?php
 foreach ($province_arr as $k => $v){ echo "<option value='$k'>$v</option>"; } ?>
              
          </select></th>
          
          <th width="152">操作</th>   
         <?php if(is_array($data)): foreach($data as $key=>$vo): ?><tbody>

            <tr>
         
              <td width="60"><input type="checkbox" class="force" value=""   id="<?php echo ($vo["id"]); ?>"></td>
              <td width="100" class="no_bl"><a target="_blank" href="#"><img width="46" height="46" src="/object<?php echo ($vo["img"]); ?>"></a></td>  
              <td style="text-align:left; padding:0 10px;" class="no_bl"><a target="_blank" href="#"><?php echo ($vo["name"]); ?></a></td>
              <td width="148"><span class="MSyh">￥</span><span class="Arial"><?php echo ($vo["price"]); ?></span></td>


              <td width="148"><span  class=" lower border pl5 pr5 reduceItem count-down" id="<?php echo ($vo['id']); ?>" >-</span>
              <input type="text" value="<?php echo ($vo["num"]); ?>" size="8" class="t_c count-free">
              <span class=" higher border pl5 pr5 addItem count-up" id="<?php echo ($vo['id']); ?>" >+</span></td>


              <td width="148"><span class="red"><?php echo ($vo["price"] * $vo["num"]); ?></span></td>
              <td width="148">有货</td>
              <td width="148"><a href="/object/index.php/Home/cart/del/id/<?php echo ($vo['id']); ?>"><span class="itemDelete blue hand product-erase">删除</span></a></td>
        ...
        <div class="red_bt_l right mt15 mr20">
          <a webtrekkparam="" id="account" >
            <span class="white block">去结算</span>
          </a>
        </div>
        <div class="right mr20 font14">
         <span class="cart_cleasTxt">总计（不含运费）：</span>
          <span class="red font24">￥</span>
          <span class=" Arial red font22 cartPirce">
          <?php echo $sum; ?>.00</span> 
        </div>
   


          <div id="clearCart" class="left clearCart dark font14 hand">
            <a webtrekkparam="{ct:'shop_myshoppingcart_delete_chosen'}" class="del_cart"></a>删除选中商品
          </div>
          <div>
          <a webtrekkparam="{ct:'shop_myshoppingcart_goonshop'}" class="left" id="goback" href="<?php echo U('Cart/clear');?>">
            <span class="ml30 font14"> 清空购物车</span>
          </a>
        </div>

          
          <div class="left clearCart dark font14 hand">
             <a href="<?php echo U('Fit/fitlist');?>"><span class="ml30 font14"> 继续购物</span></a>
          </div>
        ...
        <div id="attact">
          ...
          <!--4-->
          <div id="plus" class="cart_menu_wrap">
  ...
  <div class="center mb20 mt80" style="display:none;" id="recommend">
    <div class="cart_title line_b mt10 "><span class="font18 dark"></span></div>          
  </div>
 	
<div id="light" class="cart_div">正在转向结算页面，请稍等……</div>
<div id="fade" class="cart_overlay"></div>
...
<script type="text/javascript">
    ...
    $("#allcheck").change(function(){
      ...
      $("#clearCart").click(function(){
        var ids=[];
        var id;
        // $('.force:checked').parents("tr").remove();
        $('.force:checked').each(function(){
            console.log($(this).attr('id'));
         id=$(this).attr('id');
         ids.push(id);
        });
        console.log(ids);
        var r = confirm("确认要删除所选商品吗?");
        if(r){
          $.post("/object/index.php/Home/Cart/delete",{"id":ids},function(msg){
              if(msg==0){
                 location.reload();
              }
          });
  ...
  <script type="text/javascript">
          ...
          $(".count-down").click(function(){
            // alert(222);
              ...
              var d = {"id":id};
              var decr = $(this);
              $.post("/object/index.php/Home/Cart/decr",d,function(data){
                  if(data){
                    $this.next('input').val(data);
                    var sum = data * parseInt(decr.parent().prev().find('span[class=Arial]').html());
                    $this.parent().next().html(sum).css('color','red');
                    location.reload();
                  }
              });
          })
          ...
          $(".count-up").click(function(){           
             ...
             var $this = $(this);
              ...
              var d = {"id":id};
              var decr = $(this);
              $.post("/object/index.php/Home/Cart/incr",d,function(data){
                  if(data){
           ...
           $("#account").click(function(){
            // alert(222);
                ...
                $('.force:checked').each(function(){
                 ...
                 id=$(this).attr('id');
                 ids.push(id);
                window.location = "/object/index.php/Home/Orderinfo/index/id/"+ids;
                });
           })
           </script>
...
function pageScroll(){
    ...
    if(sTop==0){
		clearTimeout(scrolldelay);
  		  		...
  		  		<script type='text/javascript' src='/object/Public/cart/Scripts/sxf_scroll_pic.js'></script>

  <script type="text/javascript" src="/object/Public/cart/Scripts/mediav_cart.js"></script>
  <script type="text/javascript" src="/object/Public/cart/Scripts/cart.js"></script>
